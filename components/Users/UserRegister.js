...
export default class UserRegister extends Component {
    ...
    _doCheckDupl = () => {
        ...
        if (id) {
            var url = 'https://' + CommonConf.urlHost + ':'+CommonConf.port+'/ss/api/checkUserDupl';
            this.setState({
                isLoadingNow: true
            })
    ...
    _checkValidInputValues = (id, pw, pw2, email,phone,name, isNotDuplId) => {
        ...
        if (!isNotDuplId) {  //
            ...
            if (id && pw && pw2 && email && phone) {
                ...
                if (!phone) {
                    this._showInputTextErrorMsg('phone');
                    return false;
                }
                if (!name) {
                    this._showInputTextErrorMsg('name');
                    return false;
                }
            }
        }
    ...
    _doRegister = () => {
        ...
        const { id, pw, pw2, email,phone,name, isNotDuplId } = this.state;

        let isValid = this._checkValidInputValues(id, pw, pw2, email, phone,name, isNotDuplId);

        if (isValid) {
            var url = 'https://' + CommonConf.urlHost + ':'+CommonConf.port+'/ss/api/regiUser';
        ...
        return (
            <SafeAreaView forceInset={{ bottom: 'never', top: 'never' }} style={styles.container}>
                ...
                <KeyboardAwareScrollView
                    ...
                    <View style={{ alignSelf: 'center', flexDirection: 'row', width: width / 3 * 2, height: 50, marginTop: 56}}>
                        <TextInput
                            style={{
